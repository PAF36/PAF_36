...
const SingleMilestone = (props) => {
    const { itemClass, skillID, skillImg, skillTitle, courseName, courseLesson, courseEnrolled, coursePrice } = props;
	return(
        <div className={itemClass ? itemClass : 'course__item mb-30'}>
            <div className="course__thumb">
                <Link to={`/skill-feed/${skillID}`}>
                    <img src={skillImg ? require(`../../assets/images/course/${skillImg}`) : require(`../../assets/images/course/${courseImg1}`)} alt="image" />
                </Link>
            </div>
            <div className="course__inner">
                ...
                <h3 className="react-course-title">
                    <Link to={`/skill-feed/${skillID}`}>
                        {skillTitle ? skillTitle : 'Dave conservatoire is the Entirely free online.'}
                    </Link>
                </h3>
